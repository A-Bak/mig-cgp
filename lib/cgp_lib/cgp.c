...
DLL_EXPORT struct results* repeatCGP(struct parameters *params, struct dataSet *data, int numGens, int numRuns) {
	...
	for (i = 0; i < numRuns; i++) {
    	...
    	// setRandomNumberSeed(time(NULL)+i);
		/* run cgp */
...
DLL_EXPORT struct chromosome* runCGP_InitChromo(struct parameters *params, struct dataSet *data, int numGens, struct chromosome* initialChromo) {
...
struct parammeters* initParamsFromParams(struct parameters *params){

	struct parameters* newParams = initialiseParameters(params->numInputs, params->numNodes, params->numOutputs, params->arity);

	newParams->mu = params->mu;
	newParams->lambda = params->lambda;
	newParams->evolutionaryStrategy = params->evolutionaryStrategy;
	newParams->mutationRate = params->mutationRate;
	newParams->recurrentConnectionProbability = params->recurrentConnectionProbability;
	newParams->connectionWeightRange = params->connectionWeightRange;

	newParams->funcSet = params->funcSet;
	newParams->funcSet = (struct functionSet*)malloc(sizeof(struct functionSet));
	copyFuctionSet(newParams->funcSet, params->funcSet);

	newParams->targetFitness = params->targetFitness;
	newParams->updateFrequency = params->updateFrequency;
	newParams->shortcutConnections = params->shortcutConnections;

	newParams->mutationType = params->mutationType;
	strcpy(newParams->mutationTypeName, params->mutationTypeName);

	newParams->fitnessFunction = params->fitnessFunction;
	strcpy(newParams->fitnessFunctionName, params->fitnessFunctionName);

	newParams->selectionScheme = params->selectionScheme;
	strcpy(newParams->selectionSchemeName, params->selectionSchemeName);

	newParams->reproductionScheme = params->reproductionScheme;
	strcpy(newParams->reproductionSchemeName, params->reproductionSchemeName);

	newParams->numThreads = 1;

	return newParams;
}


DLL_EXPORT struct results* repeatCGP_WithOptim(struct parameters *params, struct dataSet *data, int numGens, int numGensOptim, int numRuns,
											   double (*customFitnessOptim)(struct parameters *params, struct chromosome *chromo, struct dataSet *dat)) {

	int i;
	struct results *rels_cgp;
	struct results *rels_optim;
	int updateFrequency = params->updateFrequency;

	/* set the update frequency so as to to so generational results */
	params->updateFrequency = 0;

	rels_cgp = initialiseResults(params, numRuns);
	rels_optim = initialiseResults(params, numRuns);

	printf("------------------------------------------------------------------------------------------------------\n");
	printf("Run\tFitness\t\tGenerations\tActive Nodes\tFitness Optim\tGen Optim\tActive Optim\n");
	printf("------------------------------------------------------------------------------------------------------\n");

	int targetFitness = params->targetFitness;
	double (*customFitness)(struct parameters *params, struct chromosome *chromo, struct dataSet *dat) = params->fitnessFunction;

	/* for each run */
	// #pragma omp parallel for default(none), shared(targetFitness,numRuns,rels_cgp,rels_optim,params,data,numGens,numGensOptim,customFitness,customFitnessOptim), schedule(dynamic), num_threads(params->numThreads)
	for (i = 0; i < numRuns; i++){

		srand(time(NULL)+i+omp_get_thread_num());

		struct parameters *p = initParamsFromParams(params);
		/* run cgp */

		rels_cgp->bestChromosomes[i] = runCGP(p, data, numGens);
		rels_optim->bestChromosomes[i] = initialiseChromosomeFromChromosome(rels_cgp->bestChromosomes[i]);
		rels_optim->bestChromosomes[i]->generation = 0;

		if(getChromosomeFitness(rels_cgp->bestChromosomes[i]) == p->targetFitness){

			setTargetFitness(p, targetFitness - p->numNodes);
			setCustomFitnessFunction(p, customFitnessOptim, "customFitnessOptim");

			rels_optim->bestChromosomes[i] = runCGP_InitChromo(p, data, numGensOptim, rels_optim->bestChromosomes[i]);

		}
		printf("%d\t%f\t%d\t\t%d\t\t%f\t%d\t\t%d\n", i, rels_cgp->bestChromosomes[i]->fitness, rels_cgp->bestChromosomes[i]->generation, rels_cgp->bestChromosomes[i]->numActiveNodes,
													  rels_optim->bestChromosomes[i]->fitness, rels_optim->bestChromosomes[i]->generation, rels_optim->bestChromosomes[i]->numActiveNodes);		
	}	

	printf("------------------------------------------------------------------------------------------------------\n");
	printf("MEAN\t%f\t%f\t%f\t%f\t%f\t%f\n", getAverageFitness(rels_cgp), getAverageGenerations(rels_cgp), getAverageActiveNodes(rels_cgp),
	 										 getAverageFitness(rels_optim), getAverageGenerations(rels_optim), getAverageActiveNodes(rels_optim));
	printf("MEDIAN\t%f\t%f\t%f\t%f\t%f\t%f\n", getMedianFitness(rels_cgp), getMedianGenerations(rels_cgp), getMedianActiveNodes(rels_cgp),
	 										   getMedianFitness(rels_optim), getMedianGenerations(rels_optim), getMedianActiveNodes(rels_optim));
	printf("------------------------------------------------------------------------------------------------------\n");
